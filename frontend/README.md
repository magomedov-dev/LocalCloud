...
- **TanStack Virtual 3** — виртуализация списка/сетки файлов (`FileGrid`).
...
- **Тесты** — Vitest + Testing Library + jsdom (Playwright есть в
  devDependencies, E2E-тестов в репозитории пока нет).
...
├── tests/               Vitest-тесты (api, components, contexts, hooks,
│                        lib, pages, security)
...
| `components/` (корень) | `ErrorBoundary` (ловит ошибки рендера внутри `AppShell`), `TopLoadingBar` (индикатор фоновой подгрузки). |
...
| `useThumbnails` | Батч-загрузка presigned URL миниатюр (≤100 id), кэш в sessionStorage; повторно опрашиваются только `pending` (генерируется) с backoff (4→30 с, до 8 раз), на фоновой вкладке опрос приостанавливается. Окончательный `none` («миниатюры не будет») кэшируется. Учитывает флаг `previews_enabled`. |
...
- **`files/`** — `FileGrid` (виртуализированный grid/list на TanStack Virtual,
  выбор, фильтр/сортировка, drag-drop),
...
**Миниатюры**: один батч-запрос на видимые узлы, кэш в sessionStorage.
Сервер отвечает тремя статусами: `ready` (presigned URL, кэш 12 мин), `none`
(окончательное «миниатюры не будет» — кэш 30 мин) и `pending` (генерируется —
не кэшируется и опрашивается с экспоненциальным backoff, на фоновой вкладке
...
| `THUMBNAIL_NEGATIVE_TTL_MS` | 30 мин | Кэш окончательного «миниатюры не будет» (`none`); `pending` не кэшируется. |
...
`setupFiles: ./tests/setup.ts`). Покрываются страницы, хуки, контексты,
компоненты, API-модули, утилиты `lib/` и security-сценарии (`tests/security/`).
Пороги покрытия: **statements/lines/functions 85%, branches 80%**.
