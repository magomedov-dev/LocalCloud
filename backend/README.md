...
| `core/` | `config.py` (11 групп настроек Pydantic), `constants.py` (литералы-дефолты + регулярки/лимиты), `logging.py` (настройка логирования), `rate_limit.py` (sliding-window лимитер запросов), `secret_validation.py` (проверка секретов на дефолты при старте), `preview_mime.py` (какие MIME-типы получают превью). |
...
| `storage/` | Интеграция с MinIO: `service.py` (высокоуровневый фасад), `client.py` (async-обёртка SDK), `buckets.py`, `objects.py`, `multipart.py`, `presigned.py`, `keys.py`, `metadata.py`, `capacity.py`, `integrity.py`, `health.py`, `types.py`, `exceptions.py`. |
| `workers/` | Фоновый процесс: `app.py` (точка входа), `dispatcher.py`, `scheduler.py`, `registry.py`, обработчики (`previews`, `archives`, `cleanup`, `uploads`, `public_links`, `quotas`, `integrity`), `tasks.py` (хелперы результатов/payload), `health.py` (health-checker воркера), `context.py`, `types.py`, `lifecycle.py`, `exceptions.py`. |
...
| `PUBLIC_LINK_PASSWORD_MAX_ATTEMPTS` | `5` | Неверных паролей публичной ссылки до блокировки (1–100). |
| `PUBLIC_LINK_PASSWORD_LOCKOUT_SECONDS` | `900` | Длительность блокировки проверок пароля ссылки. |
| `RATE_LIMIT_AUTH_ATTEMPTS` | `10` | Запросов к auth-точкам (логин, заявка) с одного IP за окно. |
| `RATE_LIMIT_AUTH_WINDOW_SECONDS` | `300` | Окно лимита auth-запросов. |
| `RATE_LIMIT_PUBLIC_ACCESS_ATTEMPTS` | `30` | Проверок доступа к публичным ссылкам с одного IP за окно. |
| `RATE_LIMIT_PUBLIC_ACCESS_WINDOW_SECONDS` | `300` | Окно лимита проверок публичных ссылок. |
...
| `POSTGRES_MAX_CONNECTIONS` | `null` | Лимит сервера Postgres — только для проверки согласованности пула на старте. |
| `UVICORN_WORKERS` | `1` | Число процессов uvicorn — для той же проверки. |

> Коннектов в кластере: `(UVICORN_WORKERS + 1) × (POOL_SIZE + MAX_OVERFLOW)`;
> держите ниже `max_connections` PostgreSQL с запасом ~10. При старте backend
> предупреждает в логе, если пул не вписывается в `POSTGRES_MAX_CONNECTIONS`.
...
| `MINIO_CONNECT_TIMEOUT_SECONDS` | `5.0` | Таймаут установки TCP-соединения с MinIO. |
| `MINIO_READ_TIMEOUT_SECONDS` | `60.0` | Таймаут на каждое чтение из сокета (не на весь файл). |
| `STORAGE_STARTUP_TIMEOUT_SECONDS` | `30.0` | Потолок ожидания готовности хранилища на старте. |
| `INCOMPLETE_MULTIPART_EXPIRY_DAYS` | `7` | MinIO авто-абортит незавершённые multipart-загрузки старше N дней (0 — правило не ставится). |
...
| `WORKER_NAME` | `null` | Имя worker-процесса (иначе генерируется с префиксом `localcloud-worker`). |
...
| `WORKER_INTEGRITY_CONCURRENCY` | `5` | Одновременных проверок объектов в batch-е целостности (1–64). |
...
`lifecycle.py` на старте валидирует секреты (`core/secret_validation.py`:
вне debug-режима приложение **отказывается стартовать** с дефолтными
`SECRET_KEY`/паролями; также предупреждает о небезопасных cookie и
переразмеренном пуле БД), проверяет БД/хранилище и создаёт бакеты; на
остановке освобождает пулы и storage-executor.

Чувствительные эндпоинты (логин, заявка на регистрацию, доступ к публичным
ссылкам) дополнительно ограничены rate-limit-зависимостью
(`app/dependencies.py` + `core/rate_limit.py`): скользящее окно по IP в памяти
процесса, при превышении — `429`. Лимиты — `RATE_LIMIT_*` (см. конфигурацию).
...
- **Rate limiting** (`core/rate_limit.py`) — sliding-window лимиты по IP для
  логина/регистрации (`RATE_LIMIT_AUTH_*`) и проверок публичных ссылок
  (`RATE_LIMIT_PUBLIC_ACCESS_*`); превышение → `429`. Счётчики в памяти
  процесса, поэтому при нескольких uvicorn-worker'ах суммарный лимит кратен
  их числу.
- **Lockout пароля публичной ссылки** — после
  `PUBLIC_LINK_PASSWORD_MAX_ATTEMPTS` неверных паролей проверка блокируется на
  `PUBLIC_LINK_PASSWORD_LOCKOUT_SECONDS`. Счётчик хранится в БД (общий для
  всех процессов).
- **Валидация секретов на старте** (`core/secret_validation.py`) — вне
  debug-режима приложение не стартует с дефолтными значениями
  `SECRET_KEY`, паролей БД/MinIO и т.п.

---

## Хранилище (MinIO/S3)

`storage/service.py` — высокоуровневый фасад (`StorageService`) над
`storage/client.py` — async-обёрткой блокирующего MinIO SDK: вызовы
выполняются в общем `ThreadPoolExecutor` (`STORAGE_EXECUTOR_MAX_WORKERS`),
HTTP-клиент ограничен socket-таймаутами (`MINIO_CONNECT_TIMEOUT_SECONDS`,
`MINIO_READ_TIMEOUT_SECONDS`), готовность хранилища на старте ждётся не дольше
...
- **Multipart** (`multipart.py`) — init/part/complete/abort; часть 8 МБ (мин. 5 МБ),
  до 10000 частей. На старте ставится lifecycle-правило MinIO: незавершённые
  загрузки старше `INCOMPLETE_MULTIPART_EXPIRY_DAYS` абортятся автоматически.
...
7. `a1c3e5b7d9f2` — lockout пароля публичных ссылок (счётчик неверных попыток).
...
Вне debug-режима отказывается работать с дефолтным `ADMIN_PASSWORD`.
